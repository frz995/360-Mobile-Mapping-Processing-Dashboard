...
import type { Container, Engine, ISourceOptions, SingleOrMultiple } from "@tsparticles/engine";
...
const SparklesCanvas: React.FC<{ id: string; props: SparklesProps }> = ({ id, props }) => {
  const { background, minSize, maxSize, speed, particleColor, particleDensity } = props;
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={controls} className={cn("h-full w-full")}>
      <Particles
        id={id}
  ...
  return (
    <div className={cn("relative w-full h-full", className)}>
      <ParticlesProvider init={initParticles}>
        <SparklesCanvas id={id || generatedId} props={props} />
      </ParticlesProvider>
    </div>
  );
};
