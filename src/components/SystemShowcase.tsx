...
export const SystemShowcase: React.FC<SystemShowcaseProps> = ({
    ...
    const SYSTEM_MODULES: SystemModule[] = [
        {
            id: 'webgis',
            category: 'Spatial Trajectory Telemetry',
            title: 'Interactive Coverage Map & WebGIS',
            subtitle: 'Trajectory Geometries & Subgrid Coverage Tracking',
            description: 'Monitors vehicle spatial trajectories along road networks. Synchronizes high-precision GNSS positioning coordinates, groups surveyed points into regional grid boundaries, and visualizes route coverage across standard GIS basemaps.',
            ...
            specs: [
                ...
                { label: 'Trajectory Metric', value: 'Geodesic Distance (KM)' },
            ],
        },
        {
            id: 'processing',
        ...
        },
        {
            ...
            title: 'Panoramic StreetView & Defect Workspace',
        ...
        <div className="relative w-full h-[100dvh] max-h-[100dvh] bg-[var(--bg-app,#080e1a)] text-[var(--text-primary,#f8fafc)] font-sans overflow-hidden select-none flex flex-col justify-between transition-colors duration-200">
            ...
            <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden">
                <img
                    ...
                    loading="eager"
                    decoding="async"
                    className="w-full h-full object-cover scale-125 blur-[120px] sm:blur-[150px] opacity-30 sm:opacity-40 transition-all duration-1000 ease-in-out"
                />
                <div className="absolute inset-0 bg-[var(--bg-app,#080e1a)]/80 backdrop-blur-xl transition-colors duration-300" />
            </div>
            ...
            <main className="relative z-20 flex-1 w-full px-3 sm:px-8 py-3 sm:py-4 overflow-y-auto lg:overflow-hidden flex items-start lg:items-center justify-start lg:justify-center">
                <div className="w-full max-w-[1700px] mx-auto my-0 lg:my-auto grid grid-cols-1 lg:grid-cols-12 gap-5 sm:gap-8 lg:gap-12 items-center">
                    ...
                    <div className={`w-full lg:col-span-5 space-y-4 sm:space-y-6 text-left flex flex-col justify-center order-3 lg:order-1 pb-6 lg:pb-0 transition-all duration-300 ease-out ${isAnimating ? 'opacity-0 translate-y-1.5' : 'opacity-100 translate-y-0'}`}>
                        ...
                        <div className="hidden lg:block space-y-2.5 pb-2 border-b border-[var(--border-subtle,#1e2e4a)]">
                            <h1 className="text-2xl lg:text-3xl xl:text-4xl 2xl:text-5xl font-extrabold tracking-tight text-[var(--text-primary,#f8fafc)] leading-[1.1]">
                                GeoSphere 360° Mobile Mapping Platform
                            </h1>

                            <p className="text-xs sm:text-sm text-[var(--text-muted,#94a3b8)] font-normal leading-relaxed">
                                Centralizing spatial data pipelines with high-precision trajectory tracking, PostGIS cloud synchronization, and frame-by-frame spherical QA auditing.
                            </p>
                        </div>
                        ...
                        <div className="space-y-2.5 sm:space-y-3 pt-0.5">
                            ...
                            <div className="space-y-1">
                                <h2 className="text-lg sm:text-2xl lg:text-3xl font-extrabold tracking-tight text-[var(--text-primary,#f8fafc)] leading-tight">
                                    {current.title}
                                </h2>
                                <p className="text-[11px] sm:text-xs lg:text-sm text-[var(--text-muted,#94a3b8)] font-normal leading-relaxed line-clamp-3 sm:line-clamp-none">
                                    {current.description}
                                </p>
                            </div>
                            ...
                            <div className="grid grid-cols-3 gap-1.5 sm:gap-2 pt-0.5">
                                {current.specs.map((spec, i) => (
                                    <div
                                        key={i}
                                        className="p-1.5 sm:p-2.5 rounded-xl bg-[var(--bg-inner,#162138)] border border-[var(--border-subtle,#1e2e4a)] backdrop-blur-md min-w-0"
                                    >
                                        <span className="text-[9px] xs:text-[10px] sm:text-[11px] font-medium text-[var(--text-muted,#94a3b8)] block truncate">
                                            {spec.label}
                                        </span>
                                        <span className="text-[10.5px] xs:text-xs sm:text-sm font-semibold text-[var(--text-primary,#f8fafc)] block mt-0.5 truncate">
                                            {spec.value}
                                        </span>
                                    </div>
                                ))}
                            ...
                            <div className="pt-1">
                                <button
                                    ...
                                    className="w-full sm:w-auto px-5 sm:px-6 py-2.5 sm:py-3 rounded-xl font-semibold text-xs sm:text-sm flex items-center justify-center gap-2 transition-all transform hover:brightness-110 active:scale-95 cursor-pointer shadow-lg bg-[var(--accent,#38bdf8)] text-slate-950"
                                >
                        ...
                        <div className="pt-1 text-left space-y-2">
                            ...
                            <div className="pt-2 sm:pt-3 border-t border-[var(--border-subtle,#1e2e4a)] flex flex-wrap items-center gap-x-4 sm:gap-x-6 gap-y-1.5 text-[var(--text-muted,#94a3b8)] text-[10px] sm:text-xs">
                            ...
                            {/* Copyright Notice */}
                            <p className="pt-0.5 text-[8.5px] sm:text-[9.5px] text-[var(--text-muted,#94a3b8)] opacity-70 font-normal tracking-tight select-none">
                    ...
                    >
                        {/* Ambient Underglow */}
                        <div className="absolute -inset-3 sm:-inset-6 rounded-3xl overflow-hidden pointer-events-none opacity-30 group-hover:opacity-60 blur-xl sm:blur-3xl transition-all duration-500 ease-out -z-10">
                            <img
                                src={activeImage}
                                alt="Underglow"
                                loading="eager"
                        ...
                        <div className="relative w-full aspect-[16/10] p-2.5 sm:p-4 rounded-2xl bg-[var(--bg-card,#0f172a)] border border-[var(--border-subtle,#1e2e4a)] shadow-2xl flex flex-col justify-between overflow-hidden transition-colors duration-200">

                            {/* Top Subtitle Bar */}
                            <div className="flex items-center justify-between px-1 pb-1.5 sm:pb-2 border-b border-[var(--border-subtle,#1e2e4a)]">
                            ...
                            {/* Viewport Image */}
                            <div className="relative w-full flex-1 rounded-xl bg-[var(--bg-inner,#162138)] border border-[var(--border-subtle,#1e2e4a)] overflow-hidden flex items-center justify-center my-1.5 sm:my-3">
                                <img
                                    key={activeImage}
                                    src={activeImage}
                                    alt={current.title}
            ...
            <footer className="relative z-30 w-full px-3 sm:px-8 py-2 sm:py-3 flex items-center justify-between border-t border-[var(--border-subtle,#1e2e4a)] bg-[var(--bg-card,#0f172a)]/95 backdrop-blur-md shrink-0 transition-colors duration-200">
                <button
                ...
                >
                    <div className="w-8 h-8 rounded-lg border border-[var(--border-subtle,#1e2e4a)] bg-[var(--bg-inner,#162138)] flex items-center justify-center">
