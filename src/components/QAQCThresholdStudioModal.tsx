  ...
  return (
    <div className="flex-1 p-3.5 gap-3.5 flex overflow-hidden bg-app min-h-0">
      ...
      <main className="flex-1 bg-card/90 backdrop-blur-md border border-subtle rounded-2xl flex flex-col overflow-hidden shadow-xl ring-1 ring-white/5 relative min-w-0">
        ...
        <div className="h-14 px-4 border-b border-subtle flex items-center justify-between shrink-0 bg-card relative z-10">
          ...
          <div className="flex items-center gap-2">
            {activeDefectTab !== 'gps' ? (
              <button
                ...
                className="px-3 py-1.5 rounded-xl bg-inner hover:bg-inner/80 border border-subtle text-xs font-semibold text-zinc-300 hover:text-white transition-all cursor-pointer shadow-sm active:scale-95"
              >
              ...
              <span className="text-[11px] text-zinc-500 font-mono">Haversine Geodesic Analyzer</span>
            )}
          </div>
        </div>

        {/* Viewport Frame */}
        <div className="flex-1 bg-black relative flex flex-col p-3 overflow-hidden select-none">
            ...
            {activeDefectTab === 'blur' && (
              isComparingSplit ? (
                ...
                <div className="relative w-full h-full overflow-hidden">
                  ...
                  <div className="absolute top-4 left-4 px-3.5 py-2 rounded-xl bg-zinc-950/95 border border-zinc-700/80 text-xs shadow-2xl flex items-center gap-2.5">
                    <span className="w-2 h-2 rounded-full bg-zinc-400" />
                    <span className="text-zinc-300 font-medium">Sharp Baseline</span>
                    <span className="font-mono text-zinc-100 font-bold text-sm tabular-nums">{simulatedSharpScore.toFixed(1)}</span>
                  </div>

                  {/* Right HUD: Test Scan */}
                  <div className="absolute top-4 right-4 px-3.5 py-2 rounded-xl bg-zinc-950/95 border border-zinc-700/80 text-xs shadow-2xl flex items-center gap-2.5">
                    <span className={`w-2 h-2 rounded-full ${isSimulatedBlurFlagged ? 'bg-rose-400' : 'bg-emerald-400'}`} />
                    <span className="text-zinc-300 font-medium">Simulated Scan</span>
                    <span className="font-mono font-bold text-zinc-100 text-sm tabular-nums">{simulatedBlurScore.toFixed(1)}</span>
                  ...
                  {/* Center Helper Text */}
                  <div className="absolute bottom-4 left-1/2 -translate-x-1/2 px-4 py-1.5 rounded-full bg-zinc-950/95 border border-zinc-700/80 text-xs text-zinc-300 font-sans pointer-events-none shadow-2xl">
                    Drag split divider horizontally to compare Sharp (Left) vs Blurry (Right)
                  </div>
                </div>
              ) : (
                /* Side-by-Side Dual View */
                <div className="w-full h-full grid grid-cols-2 gap-3.5 p-3.5">
                  <div className="relative rounded-xl overflow-hidden border border-subtle bg-black flex items-center justify-center">
                    <img
                      src="/samples/sample_survey_sharp.jpg"
                      alt="Sharp Reference"
                      className="w-full h-full object-contain"
                    />
                    <div className="absolute top-3 left-3 px-3 py-1.5 rounded-xl bg-zinc-950/95 border border-zinc-700/80 text-xs text-zinc-200 flex items-center gap-2 shadow-xl">
                      <span className="w-1.5 h-1.5 rounded-full bg-zinc-400" />
                      <span className="font-medium">Sharp Baseline ({simulatedSharpScore.toFixed(1)})</span>
                    </div>
                  </div>
                  ...
                  <div className="relative rounded-xl overflow-hidden border border-subtle bg-black flex items-center justify-center">
                    ...
                    <div className="absolute top-3 left-3 px-3 py-1.5 rounded-xl bg-zinc-950/95 border border-zinc-700/80 text-xs text-zinc-200 flex items-center gap-2 shadow-xl">
                      ...
                      <span className={`text-xs font-semibold ${isSimulatedBlurFlagged ? 'text-rose-400' : 'text-emerald-400'}`}>
                        {isSimulatedBlurFlagged ? '• Defect' : '• Passed'}
                      </span>
                    </div>
                  </div>
            ...
            {activeDefectTab === 'obstruction' && (
              isComparingSplitObstruction ? (
                ...
                <div className="relative w-full h-full overflow-hidden">
                  ...
                  <div className="absolute top-4 left-4 px-3.5 py-2 rounded-xl bg-zinc-950/95 border border-zinc-700/80 text-xs shadow-2xl flex items-center gap-2.5">
                    <span className="w-2 h-2 rounded-full bg-zinc-400" />
                    <span className="text-zinc-300 font-medium">Nominal Clean Sensor</span>
                    <span className="font-mono text-zinc-100 font-bold text-sm tabular-nums">132.0 Luma</span>
                  </div>

                  <div className="absolute top-4 right-4 px-3.5 py-2 rounded-xl bg-zinc-950/95 border border-zinc-700/80 text-xs shadow-2xl flex items-center gap-2.5">
                    <span className={`w-2 h-2 rounded-full ${isObstructionFlagged ? 'bg-rose-400' : 'bg-emerald-400'}`} />
                    ...
                    <span className={`px-2.5 py-0.5 rounded-lg text-[11px] font-semibold ${
                      ...
                      {isObstructionFlagged ? 'Defect Flagged' : 'Passed'}
                    </span>
                  </div>
                ...
                /* Side-by-Side Dual View */
                <div className="w-full h-full grid grid-cols-1 md:grid-cols-2 gap-3.5 p-3.5">
                  <div className="relative rounded-xl overflow-hidden border border-subtle bg-black flex items-center justify-center">
                    <img
                      src="/samples/sample_survey_sharp.jpg"
                    ...
                    <div className="absolute top-3 left-3 px-3 py-1.5 rounded-xl bg-zinc-950/95 border border-zinc-700/80 text-xs text-zinc-200 flex items-center gap-2 shadow-xl">
                      <span className="w-1.5 h-1.5 rounded-full bg-zinc-400" />
                      <span className="font-medium">Nominal Frame (Clear Sensor)</span>
                    </div>
                    <div className="absolute bottom-3 left-3 px-3 py-1.5 rounded-xl bg-zinc-950/95 border border-zinc-700/80 text-xs font-mono text-zinc-400 tabular-nums shadow-xl">
                      Luminance: 132.0 • Status: Passed
                    </div>
                  </div>
                  ...
                  <div className="relative rounded-xl overflow-hidden border border-subtle bg-black flex items-center justify-center">
                    ...
                    <div className="absolute top-3 left-3 px-3 py-1.5 rounded-xl bg-zinc-950/95 border border-zinc-700/80 text-xs text-zinc-200 flex items-center gap-2 shadow-xl">
                      ...
                      <span className="font-medium">
                        {obstructionSimMode === 'glitch' && `Glitch Simulation (${glitchIntensity}%)`}
                        {obstructionSimMode === 'blackout' && `Blackout Simulation (Luma ${blackoutSimLevel})`}
                        {obstructionSimMode === 'glare' && `Glare Simulation (Luma ${glareSimLevel})`}
                      </span>
                    </div>

                    <div className="absolute bottom-3 left-3 px-3 py-1.5 rounded-xl bg-zinc-950/95 border border-zinc-700/80 text-xs font-mono text-zinc-200 shadow-xl">
                      {isObstructionFlagged ? (
                        <span className="text-rose-400 font-semibold">• Defect Triggered</span>
                      ) : (
                        <span className="text-emerald-400 font-semibold">• Within Limits (Passed)</span>
                      )}
                    </div>
                  </div>
                </div>
            ...
            {activeDefectTab === 'gps' && (
              <div className="w-full h-full flex flex-col items-center justify-center p-6">
                <div className="w-full max-w-3xl flex flex-col items-center justify-center space-y-6">
                  ...
                  <div className="px-4 py-2 rounded-xl bg-zinc-950/95 border border-zinc-700/80 text-xs font-sans text-zinc-200 flex items-center gap-2.5 shadow-2xl">
                    <span className="w-2 h-2 rounded-full bg-zinc-400" />
                    <span>Nominal MMS Survey Pace: <strong className="text-zinc-100 font-semibold font-mono">2.5m – 4.5m / step</strong></span>
                  </div>

                  {/* SVG Trajectory Diagram */}
                  <svg className="w-full h-44" viewBox="0 0 740 180">
              ...
              <div className="space-y-1.5">
                ...
                <div className="grid grid-cols-3 gap-1.5 p-1 rounded-xl bg-inner border border-subtle text-xs">
                  <button
                    ...
                    className={`py-1.5 px-1 rounded-lg text-center font-medium transition-all cursor-pointer text-xs ${
                      thresholds.blurVarianceThreshold === 45.0
                  ...
                  <button
                    ...
                    className={`py-1.5 px-1 rounded-lg text-center font-medium transition-all cursor-pointer text-xs ${
                      thresholds.blurVarianceThreshold === 60.0
                        ? 'bg-card text-white shadow-sm font-semibold ring-1 ring-white/10'
                        : 'text-text-muted hover:text-text-base'
                    }`}
                  >
              ...
              <div className="space-y-1.5">
                ...
                <div className="grid grid-cols-3 gap-1.5 p-1 rounded-xl bg-inner border border-subtle text-xs">
                  <button
                    ...
                    className={`py-2 px-1 rounded-lg text-center font-medium transition-all cursor-pointer text-xs flex flex-col items-center gap-1 ${
                      obstructionSimMode === 'glitch'
                        ? 'bg-card text-white shadow-sm font-semibold ring-1 ring-white/10'
                        : 'text-text-muted hover:text-text-base'
                    }`}
